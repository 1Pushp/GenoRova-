...
const pricingPlans = [
  { name: "Free", price: "10 molecules/day", subtitle: "basic scoring" },
  {
    name: "Research",
    price: "$49/mo",
    subtitle: "1000 molecules, full scoring, docking",
    featured: true,
  },
...
export default function PlatformApp() {
  ...
  const bestMoleculeProperties = useMemo(
    () => [
      ...
      ["Clinical Score", stats.best_score],
      ["CA7 Ki", stats.best_ca7_ki],
      ["Molecular Weight", `${stats.best_molecular_weight} Da`],
      ["Docking", `${stats.best_docking_affinity} kcal/mol`],
    ],
    [stats]
  );
    ...
    <div className="min-h-screen bg-[#0A0F1E] text-white">
        ...
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-8">
          <div className="max-w-4xl">
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">
              Generate validated drug candidates in minutes
            </p>
          </div>
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-3">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur transition hover:border-teal-400/30 hover:bg-white/10">
              <div className="text-3xl font-semibold text-white">{stats.best_ca7_ki}</div>
              <div className="mt-2 text-sm text-slate-300">CA7 Ki</div>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
          <div className="grid gap-8 xl:grid-cols-[1.1fr,1.4fr]">
            <div className="rounded-[32px] bg-[#0A0F1E] p-8 text-white shadow-2xl">
              ...
              <h2 className="mt-4 text-3xl font-semibold">Generate Drug Candidates</h2>
              <form className="mt-8 space-y-6" onSubmit={handleGenerate}>
                ...
                >
                  {generateLoading ? <LoadingSpinner label="Generating..." /> : "Generate Drug Candidates"}
                </button>
            ...
            <div className="rounded-[32px] border border-slate-200 bg-white p-8 shadow-xl">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-2xl font-semibold text-slate-900">Generated candidate table</h3>
                </div>
                <button
                  type="button"
                  onClick={downloadCsv}
                      ...
                      {generateResults.length ? (
                        ...
                        <tr>
                          <td colSpan="6" className="px-4 py-10 text-center text-slate-500">
                            Run the live demo to load candidate molecules here.
                          </td>
                        </tr>
                      )}
                    </tbody>
              ...
              <div className="mt-6">
                {scoreData ? (
                  ...
                  <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-6 py-12 text-center text-slate-500">
                    Score a molecule to see MW, LogP, QED, SA Score, Lipinski, and Clinical Score.
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
          <div className="rounded-[36px] border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur">
            <div className="grid gap-8 lg:grid-cols-[1.4fr,1fr]">
              <div>
                <div className="text-sm font-semibold uppercase tracking-[0.24em] text-teal-300">Best Molecule Discovered</div>
                <h2 className="mt-4 max-w-3xl text-3xl font-semibold text-white sm:text-4xl">
                  {stats.best_molecule}
                </h2>
                <p className="mt-6 max-w-2xl text-base leading-7 text-slate-300">
                  This molecule was computationally validated with real AutoDock Vina molecular docking
                </p>
                <div className="mt-8 grid gap-4 sm:grid-cols-3">
                  ...
                  <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
                    <div className="text-sm text-slate-400">CA7 Ki</div>
                    <div className="mt-2 text-2xl font-semibold text-white">{stats.best_ca7_ki}</div>
                  </div>
                  <div className="rounded-3xl border border-white/10 bg-white/5 p-5">
                    <div className="text-sm text-slate-400">MW</div>
                    <div className="mt-2 text-2xl font-semibold text-white">{stats.best_molecular_weight} Da</div>
                  </div>
                </div>
              </div>
      ...
      <footer className="border-t border-white/10 bg-[#0A0F1E]">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-6 py-8 text-sm text-slate-300 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div>Built by Pushp Dwivedi</div>
